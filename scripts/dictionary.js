...
// almacenar la categoría activa
let activeCategory = "all";
let activeDictionary = [];

// Actualizar tabla del diccionario
function updateDictionaryTable() {
  ...
  const words =
    activeCategory === "all"
      ? Object.values(dictionary.categories).flat()
      : dictionary.categories[activeCategory];

  activeDictionary = words; // Guardar palabras actuales para ordenar
...
// Función para ordenar palabras por inglés o español
function sortDictionary(byLanguage) {
  activeDictionary.sort((a, b) =>
    a[byLanguage].localeCompare(b[byLanguage], "en", { sensitivity: "base" })
  );
  // Actualizar la tabla con las palabras ordenadas
  updateDictionaryTable();
}

// Escuchar clic en los botones de ordenación
document.getElementById("sortByEnglish").addEventListener("click", () => {
  sortDictionary("english");
});

document.getElementById("sortBySpanish").addEventListener("click", () => {
  sortDictionary("spanish");
});
...
document.getElementById("translateBtn").addEventListener("click", () => {
  ...
  Object.values(dictionary.categories).flat().forEach((word) => {
    if (language === "enToEs" && word.english.toLowerCase() === inputWord) {
      translation = word.spanish;
    } else if (language === "esToEn" && word.spanish.toLowerCase() === inputWord) {
